...
微信用户在一个 weapp 上的唯一识别码是 openid。

server 端要想获取到这个 openid，流程如下（用户注册或鉴权）：

1. weapp 通过 wx.login（Taro.login）获取临时 code，发送给 server
2. server 通过这个 临时 code，以及 appid、secret 等信息发送请求获取 openid（https://developers.weixin.qq.com/miniprogram/dev/OpenApiDoc/user-login/code2Session.html）
3. server 获取用户的 openid 后
   - 如果还没有用户信息，创建用户的信息数据，并赋予一个内部的id值，作为用户在 server 端的唯一识别 id，用于生成 token 的值等
   - 如果有用户信息，直接凭唯一识别 id，生成 token
4. weapp 端获取 token，本地存储，发送请求时带上 token。如果过期了，就自动获取新 token，重新发送请求



更多说明：

- 在获取 openid 时，正常返回 errcode 是 0，错误时返回 errcode 有值
...
前端 发送获取 token 请求，server 凭唯一识别 id
   ...
   export class JwtStrategy extends PassportStrategy(Strategy) {
     constructor() {
       super({
         ...
         ignoreExpiration: true, // true 是忽略过期，false 是不忽略过期
         secretOrKey: privateKey, // 校验，这里要和创建 token 时的 key 一致
       })
     }
...
4. `auth.module.ts` 文件代码：
   ...
   @Module({
     imports: [
       PassportModule,
       JwtModule.register({secret: privateKey, signOptions: { expiresIn: 15 }})  // expiresIn 的类型是 string | number，number 时单位为秒，string 可以是 '2 days' '10h' '7d'
     ],
